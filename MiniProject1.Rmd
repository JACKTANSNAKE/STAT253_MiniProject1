...
#model_coef 
#This is a long list of coeffs and we do not want them listed up in the report.
...
We attempted the LASSO model on log-transformed `price`. By doing this, we were able to increase our $R^2$ from 0.507 to 0.595 and improve our residual plot.



### Further refine the model - Least Square method 
...
In addiiton to LASSO and least squares, we tried parametric models of GAM and KNN that might better predict `price`. As both models performed worse than our least square models in terms of strength (measured by $R^2$) and accuracy of predictions (measured by CV MAE), we decided to settle for the least-square model.
...
## Appendix on Parametric Model Trials
...
Originally, the GAM algorithm could not run on our airbnb_complete_5000 dataset. We thought the large number of catergorical variables (and reference levels) prevented it from processing. Thus, we removed the less significant variables according to the least squares model. After trial and error, we found that the only variable that GAM had a problem with was `room_type`, so we removed this variable.
